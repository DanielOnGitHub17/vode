...
def role_rounds(request, role_id):
    ...
    return render(request, 'swe/role_rounds.html', {
        'role': role,
        'rounds': rounds
    })
...
def round_edit(request, round_id):
    ...
    if request.method == "POST":
        ...
        round_obj.difficulty_level = request.POST.get('difficulty_level', round_obj.difficulty_level)
        round_obj.time_limit = int(request.POST.get('time_limit', round_obj.time_limit))
        round_obj.data_structures = request.POST.get('data_structures', round_obj.data_structures)
        round_obj.success_metrics = request.POST.get('success_metrics', round_obj.success_metrics)
    ...
    return render(request, 'swe/round_edit.html', {
        'round': round_obj,
        'role': round_obj.role,
